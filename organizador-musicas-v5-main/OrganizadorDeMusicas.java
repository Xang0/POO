...
public class OrganizadorDeMusicas
{
    ...
    public void tocarMusica(int indice)
    {
        if(indiceEhValido(indice)) {
            ...
            musicas.get(indice).incrementaContagemExecucoes();
        }
    }
    ...
    /**
     * Le musicas que estao no arquivo
     */
    ...
    /**
     * Remove todas as muusicas com uma certa string no titulo
     */
    public void removerMusica(String nome)
    {
        int indice = 0;
        while (indice < obterQuantidadeDeMusicas())
        {
            if (musicas.get(indice).obterTitulo().contains(nome))
            {
                musicas.remove(indice);
                indice += 1;
            }
